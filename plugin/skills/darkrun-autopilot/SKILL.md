...
Drive the Run hands-off: call `darkrun_run_next` in a loop and do exactly what each returned action says, with `ask` Checkpoints auto-approved so the lifecycle advances on its own. The manager owns Checkpoint promotion — don't flip gates by hand.
