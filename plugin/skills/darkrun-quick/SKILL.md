...
A quick task is an ordinary Run right-sized down to the build + prove stations with auto gates. Create it with `darkrun_run_start { slug, title, factory, mode: "quick" }`, then drive it with `darkrun_tick` until it seals — do exactly what each returned action says.
