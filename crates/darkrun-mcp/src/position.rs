...
pub fn derive_position(store: &StateStore, slug: &str) -> Result<Position> {
    ...
    let action = match phase {
        ...
        StationPhase::Checkpoint => {
            ...
            // tick. Gated on Auto only: solo/team park at their gate instead,
            // where `checkpoint_decide`'s evidence gate enforces the same
            // guarantee on the human approve (with an explicit override).
