...
fn vital_card(v: &VitalMetric) -> Element {
    let (tone, verdict_label) = match v.verdict {
        VitalVerdict::Good => (tokens::STATUS_OK, "good"),
        VitalVerdict::NeedsImprovement => (tokens::STATUS_WARN, "fair"),
        VitalVerdict::Poor => (tokens::STATUS_DANGER, "poor"),
        VitalVerdict::Unknown => (tokens::TEXT_FAINT, "—"),
    };
    let card = format!(
...
fn audit_row(a: &AuditRow) -> Element {
    ...
    rsx! {
        div { class: "dr-audit", style: "{row}",
            ...
            span { style: format!("flex:1;color:{};", tokens::TEXT), "{a.name}" }
            span { style: format!("color:{};", tokens::TEXT_MUTED), "{a.value}" }
