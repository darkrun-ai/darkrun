...
> **Run** `darkrun-sim` · **Station** `build` · **Phase** `fix_feedback`
...
5. **Close the loop**: record what you changed and why on `fb-08`, and resolve it.

## Done when

`fb-08` is resolved with evidence, the artifact is corrected, and nothing else regressed. Then call `darkrun_tick` — if more feedback is open, the manager routes the next item; otherwise the run resumes.