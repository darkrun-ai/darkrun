...
> **Run** `darkrun-sim` · **Station** `frame` · **Phase** `review`
...
Persist it: call `darkrun_brief_record` with `slug: darkrun-sim`, `station: frame`, `phase: pre`, and the brief as `body`. This is the pre-execution brief the operator's review gate surfaces — a durable artifact, not inline prose.
