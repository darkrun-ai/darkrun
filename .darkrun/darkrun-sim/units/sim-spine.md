...
- name: seam-greps
  command: sh -c '! grep -n "[^_]run_tick(" crates/darkrun-sim/src/harness.rs crates/darkrun-sim/src/world.rs && ! grep -n "\.action" crates/darkrun-sim/src/provider.rs && ! grep -n "fn capture_to_seal" crates/darkrun-sim/src/harness.rs && grep -c "fn capture_to_seal" crates/darkrun-sim/src/scenarios.rs | grep -qx 1 && test "$(grep -c \"\\.action\" crates/darkrun-sim/src/world.rs)" = "$(sed -n \"/fn grade_tick/,/^}/p\" crates/darkrun-sim/src/world.rs | grep -c \"\\.action\")"'
...
4. Seam greps all hold: no bare `run_tick(` in harness.rs/world.rs; ZERO `.action` occurrences in provider.rs (doc comments included); every `.action` occurrence in world.rs falls inside the single `fn grade_tick` body; `fn capture_to_seal` absent from harness.rs and present exactly once in scenarios.rs.
