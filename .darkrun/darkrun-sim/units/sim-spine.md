...
- name: seam-greps
  command: sh -c '! grep -n "[^_]run_tick(" crates/darkrun-sim/src/harness.rs crates/darkrun-sim/src/world.rs && ! grep -n "\.action" crates/darkrun-sim/src/world.rs crates/darkrun-sim/src/provider.rs && ! grep -n "fn capture_to_seal" crates/darkrun-sim/src/harness.rs && grep -c "fn capture_to_seal" crates/darkrun-sim/src/scenarios.rs | grep -qx 1'
...
2. The five named tests exist and pass: `cargo test -p darkrun-sim scripted_provider_ignores_prompt_content state_reads_resolve_targets_only regenerate_twice_is_byte_equal escalate_scenario_is_detected_red committed_fixture_matches_regeneration` — each runs (not filtered to zero).
3. `cargo test -p darkrun-sim fixture::` runs at least one test (AC-7's filter shape).
4. Seam greps all hold: no bare `run_tick(` in harness.rs/world.rs; zero `.action` occurrences in world.rs and provider.rs; `fn capture_to_seal` absent from harness.rs and present exactly once in scenarios.rs.
5. `cargo clippy -p darkrun-sim --all-targets -- -D warnings` exits 0.
6. `crates/darkrun-sim/fixtures/dark-core.json` exists, parses, `schema_version == 1`, `outcome == "sealed"` (or the exact serde tag Contract 3's enum produces — match the schema), non-empty `ticks` and `units`.
