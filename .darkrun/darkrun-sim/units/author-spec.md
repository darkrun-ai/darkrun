...
2. `## Acceptance criteria` — numbered `### AC-1` … `### AC-n` (n >= 12), each: one yes/no claim, the literal command or observable that checks it, and which build phase (1/2/3 from the frame) it lands in. Cover at minimum: world construction (bare TempDir, dark mode, no network), provider trait shape + scripted impl's prompt-blindness, prompt-only seam (zero `.action` reads in the driving path — checkable via grep over the new modules), transcript projection (three streams, merge rule, schema_version, normalization), fixture determinism (regenerate-twice byte-equal), red-on-Escalate detection, defined behavior for every non-Escalate failure shape, linter partition intact (existing followability tests still pass), replay route renders the committed fixture, CI gates (regenerate-diff green job + negative Escalate scenario), wasm boundary honored (web/site gains no darkrun-mcp edge — checkable via `cargo tree`).
3. `## Contracts` — the provider trait (name, method signatures, what the scripted impl may and may not condition on), `NoopHosting` (the three methods and their return values), the darkrun-core fixture schema (struct names, fields, `schema_version` field, normalization rules enumerated), the crate module map after partition (which files exist, which are rebuilt, which are untouched), the fixture file's committed path and embedding mechanism.
...
2. All six required sections present in order → `grep -c '^#' .darkrun/darkrun-sim/specify/spec.md` reports >= 6 and a manual heading-order check matches the list above.
...
6. No hedging or placeholders as criteria or boundaries: no `TBD`, `should`, `probably`, `might`, `etc.`; every AC names its checkable condition. No time estimates anywhere — sequence only.
